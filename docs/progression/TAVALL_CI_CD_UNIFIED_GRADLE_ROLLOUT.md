...
| `/srv/dev-storage/tavall-cache/shared-ci/gradle` | Active `DEVELOPMENT_SHARED` runner cache | Active provider cache; retained. |
...
The existing DEVELOPMENT Environment was re-resolved to current exact heads and preserves its established identity and policies. It remains **BLOCKED**: CONTROL reports WORKSPACE and DEVELOPMENT_TOOL as `UNKNOWN`. A read-only repository operation returns `STALE_VERSION` for the environment-owned `TavallStudios/tavall-ci` path because that clean checkout is at `ef59a10f97668bf38f47616649353e8cca1199bb` while the snapshot requests `f3ad7942eb775a232e1e92a91a6baf101cac1444`. The old commit is an ancestor of the requested commit and the origin/branch match. PR #395 adds fail-closed recovery that advances only a clean, same-origin, same-branch fast-forward; dirty or divergent checkouts remain blocked. The installed Agent has not been updated to that PR head, and its job command does not expose the new serialized-plan input. The effective systemd unit runs `/opt/tavall-cloud/tavall-cloud-agent.jar` at SHA-256 `b3edfd20…`; no matching object exists in the current immutable Cloud artifact store, and the service registry has no immutable Agent bootstrap lineage.

The default-branch `control-agent-bootstrap.yml` is an existing, owner-label-gated recovery workflow that checks an exact revision, runs `:tavall-cloud-node:test`, builds the Agent JAR, checks its SHA-256, and provides backup/rollback. Its last four runs on 2026-09-21 reached the activation step and failed because the runner user `tavall-github-runner` cannot use `sudo` to write `/opt/tavall-cloud` or restart CONTROL. The latest run `35649180862` did not replace the Agent. The runner also cannot invoke CONTROL itself: `/run/tavall-cloud/agent.sock` is group-readable only by `tavall-cloud`, the installed CLI JAR is likewise unreadable to the runner, and the current CLI has no artifact-import command (its `storage` surface is inspect/list/mount/snapshot/unmount). I did not grant runner sudo, widen socket access, create another issue/label event, or retry the workflow; it is a fenced bootstrap fallback, not TCI artifact evidence. Do not merge PR #395's workflow deletion until the immutable TCI/Cloud bootstrap path is validated.
