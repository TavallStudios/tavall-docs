...
- **VERIFIED IN PLAN:** Tavall CI's current planner accepted `.tavallci/ci.yaml` from 36 active onboarding PR heads at their exact source SHAs. This includes all seven designated remote-only repositories; none received a canonical local workspace. PR branches are unmerged and the plan result does not claim a build or artifact.
