...
- The lane has one existing DEVELOPMENT Environment; no alternate Environment with the rollout snapshot is available. It was re-resolved to `be3dd4cd5c6920788aa68581c4e2329080f2fca6c9f859fa0fe930bada63fc48` to bind TCI PR #14 head `ad695b3` and Cloud PR #395 head `0df90c24`, preserving DURABLE / DEDICATED / SHARED / inherited resource policy.
...
- The Environment-owned Cloud checkout is clean at `main@2f1c94b47d38d86a1724067c0ab8be39bd512178`, while the current snapshot pins `tavall-cloud@0df90c241a7fb601314c6cae09512c5af0fe6b1d`. The Architecture Tests shared dependency is present clean at the exact pinned `c0863bfe9e3ea38e4872eb295856e69bed30eb28`.
...
- At Cloud PR #395 head `56fb9a66`, the changed production classes compiled against available module outputs; that checkpoint did not include Cloud tests or live deployment validation.
...
- Cloud already defines Redis-backed `CloudServiceRuntime`/BLUE/GREEN records and a stable-port service router. The immutable deployment provider still activates one shared `current` release and restarts the service's single unit; the authority handler derives runtime slot/traffic/environment from static service labels. The live plugin remains `runtime.slot=SINGLE` while its template requests `A_B`, so the template is not wired to deployment-time runtime slots.
- Cloud PR #395 head `0df90c24` now passes local `:tavall-cloud:test` (312 tests, 2 skipped) and `:tavall-cloud-node:test` (249 tests). These local tests cover the router foundation and current node provider; they do not prove immutable per-slot deployment or live traffic switching.
...
## Cloud source-test checkpoint — 2026-09-26
- Cloud PR #395 head `0df90c24` passes `:tavall-cloud:test` with 312 tests and 2 skipped sandbox tests, and `:tavall-cloud-node:test` with 249 tests and no failures. The Cloud fixes preserve the PRIMARY repository identity in Console sandbox arguments and allow Agent startup without the optional Redis developer-job projection when that capability is not configured.
- The Cloud router foundation exists, but the service deploy provider still uses one shared current release and static slot labels. The TCI deployment adapter and Cloud per-runtime deployment-unit integration remain unimplemented; no live deployment evidence was produced.
</content>
</page>
