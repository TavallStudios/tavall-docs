...
- Cloud PR #395 commit `a6a34717` aligns Cloud's metadata store to TCI's `services/<service-id>` template identity. Its tests remain unrun. The installed Agent is older and its `service cd inspect` response does not expose template identity/runtime fields, so Cloud hot-load of this new template is not yet verified.
