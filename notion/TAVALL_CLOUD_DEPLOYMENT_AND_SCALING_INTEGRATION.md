...
- A hot-loaded `tavall-cloud-control/.tavallcd/cd.yaml` template now binds runtime `control-plane-agent` to immutable artifact name `tavall-cloud-agent`, requires the six Cloud CI checks, targets DEVELOPMENT/STAGING/PRODUCTION, and uses `SINGLE` production traffic for this CONTROL_PLANE runtime. Canonical template identity is `services/tavall-cloud-control`; template digest is `5033fa2b198c3f6c47065eb3e2db202d9203048a350fcda9a240c195b570c9fa`. The current TCI template loader parsed the live definition successfully.
- Cloud PR #395 commit `a6a34717` aligns Cloud's template identity with the TCI loader's `services/<service-id>` identity. Its tests remain unrun at the current head.
