...
- Cloud PR #395 commit `a6a34717` aligns Cloud's metadata store to TCI's `services/<service-id>` template identity; current head `56fb9a66` removes GitHub URLs, Gradle module names, and mutable output paths as deployment requirements. Immutable deployment lineage now requires the canonical source repository, template-selected artifact, CI run/evidence, delivery bundle, and template digest. Cloud tests remain unrun at the current head; the changed production classes compile against available module outputs.
...
- Cloud's current managed provider switches a single `current` release pointer. It does not yet hold BLUE/GREEN runtime slots or stable-service router traffic state. The plugin's `productionTrafficMode: A_B` template therefore remains unfulfilled implementation intent; no production traffic change was made.
